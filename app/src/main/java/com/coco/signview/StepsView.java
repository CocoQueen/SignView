...
public class StepsView extends View {
     ...
     * 数据源
     */
    private List<SignBean> mStepBeanList;
    ...
    private void drawSign(Canvas canvas) {
        for (int i = 0; i < mCircleCenterPointPositionList.size(); i++) {
            ...
            if (i != mCircleCenterPointPositionList.size() - 1) {
                //最后一条不需要绘制
                if (mStepBeanList.get(i + 1).getState() == SignBean.STEP_COMPLETED) {
                    //下一个是已完成，当前才需要绘制绿色
            ...
            SignBean stepsBean = mStepBeanList.get(i);
            ...
            if (i == mPosition && mCount == ANIMATION_TIME) {
                ...
                if (stepsBean.getState() == SignBean.STEP_UNDO) {
                    mDefaultIcon.setBounds(rect);
                    mDefaultIcon.draw(canvas);
                } else if (stepsBean.getState() == SignBean.STEP_CURRENT) {
                    mAttentionIcon.setBounds(rect);
                    mAttentionIcon.draw(canvas);
                } else if (stepsBean.getState() == SignBean.STEP_COMPLETED) {
                    mCompleteIcon.setBounds(rect);
                    mCompleteIcon.draw(canvas);
                }
            }
            ...
            //绘制图标
            if (stepsBean.getState() == SignBean.STEP_COMPLETED || (i == mPosition
    ...
    private void drawUnSign(Canvas canvas) {
        for (int i = 0; i < mCircleCenterPointPositionList.size(); i++) {
            ...
            //绘制增加的分数数目
            if (stepsBean.getState() == SignBean.STEP_COMPLETED) {
                //已经完成了
                if (i == mMax[0] || i == mMax[1]) {
                    //是up的需要橙色
                    mTextNumberPaint.setColor(mCurrentTextColor);
     ...
     */
    public void setStepNum(List<SignBean> stepsBeanList) {
